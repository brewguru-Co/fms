...
function InfoCard(props) {
  const { startedAt, finishedAt, day, hour, minute, second, tank, tea, onStart, onFinish } = props;
  ...
  useEffect(() => {
    setSDisabled(Boolean(startedAt));
  }, [startedAt]);
    ...
    <Card>
      ...
      <CardFooter stats>
        <div className={classes.stats}>
          <div>
            <DateRange />
            {`${startedAt ? format(startedAt * 1000, 'yyyy-MM-dd HH:mm') : ''} ~
            ${finishedAt ? format(finishedAt, 'yyyy-MM-dd HH:mm') : ''}`}
          </div>
          <div>
            <Button size='small' variant='outlined' onClick={handleStart} disabled={sDisabled}>
              시작
            </Button>
            <Button size='small' variant='outlined' onClick={handleFinish} disabled={fDisabled}>
              종료
            </Button>
          </div>
        </div>
